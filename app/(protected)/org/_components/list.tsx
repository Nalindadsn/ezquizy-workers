...
import { File, Pencil, ShieldCheck, Verified } from "lucide-react";
...
export default async function Language({ id, e_id,items }: { id?: any, e_id?: any ,items?:any}) {
  ...
  return (
    items && items.length > 0 ? (<>
      <Card>
        <CardHeader>
          <CardTitle><Link href={`/dashboard/users/${id}/proficiency`}>Sub Accounts</Link> </CardTitle>
        </CardHeader>
        <CardContent>
          <div style={{ overflowX: "scroll" }} className="w-full ">
            <table className="w-full rtl:text-right">
              ...
              <tbody>
                {items.map((item:any) => {
                  return <><tr key={item.id} className={`${e_id == item.id ? "bg-yellow-200 hover:bg-yellow-200" : "bg-white hover:bg-gray-50"} border-indigo-200  border-b dark:bg-indigo-800 dark:border-indigo-700  dark:hover:bg-indigo-600`} >
...
<span className="text-xs text-gray-500">{item?.id}</span><br/>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <div className="h-2.5 w-2.5 rounded-full bg-green-500 me-2"></div> {item.description}
                    </div>
                  </td>
...
<div className="flex">
                    ...
                    <Button size="sm" className="mr-2" asChild>
                      <Link href={`/org/${item.id}`}><Pencil  className="w-3 h-3 -mt-1 mr-1" /> EDIT</Link>
                    </Button>
