...
const EditAnswerForm = (props:any) => {
  ...
  const trueAnswer= props.answersArray.filter((p:any)=>p?.trueOrFalse==true)
  const trueAnswer1= trueAnswer.filter((p:any)=>p?.id==answer.id)
  const trueAnswer2= props.answersArray.filter((p:any)=>p?.name==answer.name)
  const trueAnswer3= props.answersArray.filter((p:any)=>p?.name==answer.name && p?.id!==answer.id)
  ...
  return (
    <form
      onSubmit={event => {
        ...
        if((!props.editing && trueAnswer2.length>0) || props.editing && trueAnswer3.length>0 ){
          toast.error(`Cannot be duplicate answers`);
        }else{
          
       if (trueAnswer.length>0 && answer.trueOrFalse==true && trueAnswer1.length<1 ) {
...
if (answer.trueOrFalse=="1" || answer.trueOrFalse==true) {
  answer.trueOrFalse=true
        ...
        props.editing ? props.updateAnswer(answer.id, answer) : props.addAnswer(answer);
        resetAddAnswer();
       }
        }
...
<div className="flex gap-2 items-center mt-3">
      ...
      <div>
      {props.editing ? answer?.trueOrFalse==true ? <select
        name="trueOrFalse"
        value={answer.trueOrFalse}
        onChange={handleInputChange}>
      
        <option value={1}  className={trueAnswer.length>0?"text-red-500 ":""}>True</option>
      ...
      <Button >{props.editing ? <Pen className="w-4 h-4"/> : <Plus className="w-4 h-4"/>}</Button>
      {props.editing && (
        <Button onClick={resetAddAnswer} variant="destructive" className="button muted-button">
          X
        </Button>
      )}
      </div>
