      ...
      <Card key={data?.id}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className='text-right'>
            ...
            {data?.quizzesSection.length} Sections<br />
...
{data?.quizzesSection.length<1&& <div className="flex justify-center items-center  text-center mt-5"><FileWarning/> No data</div>}
{data?.quizzesSection.map((x:any)=>{
  return <div key={x?.id} className="flex gap-2 border  mb-1 p-2 items-center">
              <div className="w-full ">{x?.name}
